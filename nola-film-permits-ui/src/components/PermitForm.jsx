...
const PermitForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: 500, margin: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <input type="date" name="shootDate" value={form.shootDate} onChange={handleChange} required />
      <input type="text" name="location" placeholder="Location" value={form.location} onChange={handleChange} required />
      <textarea name="specialRequests" placeholder="Special Requests" value={form.specialRequests} onChange={handleChange} rows="4" />

      <button type="submit">Submit Permit</button>

      {status === 'success' && <p style={{ color: 'green' }}>✅ Permit submitted successfully!</p>}
